...
class CFM(GaussianDiffusion):
    def forward(self, txt_tokens, ph2word, word_len, timesteps=25, cfg_scale=3.0,  mel2word=None, mel2ph=None, spk_embed=None, infer_spk_embed=None, f0=None,
        ...
        if not infer:
            ...
            neural_ode = NeuralODE(self.ode_wrapper(cond, self.K_step), solver=solver, sensitivity="adjoint", atol=1e-4, rtol=1e-4)
    ...
    def p_losses(self, x_start, t, cond, noise=None, nonpadding=None):
        ...
        if len(t_cont.shape)==0:
            t_cont = t_cont.unsqueeze(0)
        ...
        if self.loss_type == 'l1':
            ...
            if nonpadding is not None:
                if hparams['mel_loss_scale'] == 'lognorm':   
...
class CFM_Postnet(CFM):
    def forward(self, cond, ref_mels, coarse_mels, ret, infer, noise=None, solver='euler'):
        ...
        if not infer:
            ...
            neural_ode = NeuralODE(self.ode_wrapper(cond, self.num_timesteps), solver=solver, sensitivity="adjoint", atol=1e-4, rtol=1e-4)
            t_span = torch.linspace(0, 1, self.K_step + 1)
            eval_points, traj = neural_ode(x0, t_span)
            x = traj[-1]
            x = x[:, 0].transpose(1, 2)
...
class CFM_CFG(GaussianDiffusion):
    def forward(self, txt_tokens, ph2word, word_len, timesteps=25, cfg_scale=3.0,  mel2word=None, mel2ph=None, spk_embed=None, infer_spk_embed=None, f0=None,
                uv=None, tgt_mels=None, infer=False, note_tokens = None, note_durs = None, note_types = None, note2words = None, mel2notes=None,
                solver='euler', **kwargs):
        b, *_, device = *txt_tokens.shape, txt_tokens.device
...
class CFM_CFG_Postnet(CFM):
    def forward(self, cond, ref_mels, coarse_mels, ret, infer, ucond, noise=None, cfg_scale=1.0, solver='euler'):
        b, *_, device = *cond.shape, cond.device
