...
class GaussianMultinomialReflow(torch.nn.Module):
    def __init__(
            self,
            num_classes,
            denoise_fn,
        ...
        super(GaussianMultinomialReflow, self).__init__()
    ...
    def reflow_q_sample(self, x_start, t, noise=None):
        if noise==None:
        ...
        if hparams['flow_qsample'] == 'sig':
            # print(t_unsqueeze.shape, x1.shape, x0.shape)
    ...
    def reflow_p_losses(self, x_recon, ut=None, nonpadding=None):
        # print(x_recon.shape,  nonpadding.unsqueeze(dim=1).shape)
        loss = ((ut - x_recon).abs() * nonpadding.unsqueeze(dim=1)).sum() / (nonpadding.unsqueeze(dim=1) + 1e-8).sum()
        return loss

    @torch.no_grad()
    def reflow_p_sample(self, x, t, dt, k1, clip_denoised=True, repeat_noise=False, dyn_clip=None):
        # print(x.shape, dt.shape, k1.shape)
        if clip_denoised:
            x_recon = (1 - t / self.num_timesteps) * k1 + x
            if dyn_clip is not None:
                x_recon.clamp_(dyn_clip[0], dyn_clip[1])
            else:
                x_recon.clamp_(-1., 1.)
            x_tp1 = x_recon - (1 - t / self.num_timesteps - dt) * k1
        else: 
            x_tp1 = x + dt * k1
        return x_tp1
    ...
    def mixed_loss(self, f0, uv, cond, nonpadding, out_dict):
        ...
        x_num_t = self.reflow_q_sample(x_num, t, noise=noise)
        # if x_cat.shape[1] > 0:
        ...
        loss_gauss = self.reflow_p_losses(model_out_num, ut, nonpadding=nonpadding * (uv == 0).float())
    ...
    def sample(self, cond, shape, device, out_dict, dyn_clip):
        ...
        step_len = 1.0 / self.K_step
        for i in tqdm(reversed(range(0, self.K_step)), desc='sample time step', total=self.K_step):
            i = i / self.K_step * self.num_timesteps
            ...
            z_f0 = self.reflow_p_sample(z_f0, t, step_len, model_out_num, clip_denoised=hparams['f0_sample_clip'], dyn_clip=dyn_clip)
