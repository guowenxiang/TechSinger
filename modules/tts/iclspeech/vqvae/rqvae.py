...
class VQEmbeddingEMA(nn.Module):
    ...
    def encode(self, x):
        ...
        #     f.write(str([x.shape,self.embedding.shape]))
...
class RQBottleneck(nn.Module):
    ...
    def encode_indice(self, x):
        ...
        for i in range(self.code_shape[-1]):
            code = self.codebooks[i].find_nearest_embedding(residual_feature)
    ...
    def forward(self, x):
        ...
        quantized_ = quants_trunc.view_as(x)
        quants_trunc = quantized_.permute(0, 2, 1).contiguous()
    ...
    def embed_partial_code(self, code, code_idx, decode_type='select'):
        ...
        if decode_type == 'select':
            embeds = embeds[code_idx].view(B, h, -1)
